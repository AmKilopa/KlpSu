      ...
      <div className="space-y-3" role="list" aria-label="Список коротких ссылок">
        {variantItems.map(({ variant, index, timeLeft, isExpired, isMaxed, clicksPercent }) => (
          <div
            key={index}
            role="listitem"
            className={`border rounded-2xl p-3 sm:p-4 shadow-md shadow-black/40 ${
              isExpired || isMaxed
                ? 'bg-zinc-950/60 border-zinc-800'
                : 'bg-gradient-to-r from-zinc-950 to-zinc-900 border-emerald-500/30 hover:border-emerald-500/60'
            ...
            <div className="space-y-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {variant.hasPassword && (
                    <div className="flex-shrink-0" title="Защищено паролем">
                      <Lock className="w-4 h-4 text-yellow-500" aria-label="Защищено паролем" />
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-wrap gap-2 sm:gap-3 items-center">
                  ...
                  {isExpired && (
                    <span className="text-xs text-red-400 font-medium" role="status">
                      Истек
                    </span>
                  )}
                  {isMaxed && (
                    <span className="text-xs text-yellow-400 font-medium" role="status">
                      Лимит
                    </span>
                  )}
                </div>
              ...
              <div className="flex gap-2 flex-wrap">
                <button
                  ...
                  className={`flex-1 min-w-[140px] px-3 py-2 rounded-lg font-medium transition-colors border min-h-[44px] flex items-center justify-center gap-2 touch-manipulation ${
                    copiedIndex === index
                      ...
                      : 'bg-zinc-900 border-zinc-800 text-gray-200 hover:bg-zinc-800 active:bg-zinc-700'
                  }`}
                >
                  {copiedIndex === index ? (
                    <>
                ...
                <button
                  ...
                  className="px-3 py-2 rounded-lg font-medium transition-colors border border-zinc-800 bg-zinc-950 text-gray-200 hover:bg-zinc-900 active:bg-zinc-800 disabled:opacity-50 disabled:cursor-not-allowed min-h-[44px] touch-manipulation"
                >
